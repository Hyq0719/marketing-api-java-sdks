...
import com.hyq0719.mktapi.generator.tencent.entity.*;
...
public class CodeGenerator {
  ...
  public static GeneratorConfigEntity getConfig() {
    GeneratorConfigEntity generatorConfigEntity;
    try {
      Yaml yaml = new Yaml();
  ...
  public static List<String> getTemplates() {
    ...
    GENERATOR_ENTITY.getGeneratorJavaMap().forEach((k, v) -> templates.add("template" + File.separator + k + ".java.vm"));
    return templates;
  }
  ...
  public static void parseResponseMap(Map<String, List<BaseEntity>> map, List<SingleFileEntity> singleFileList,
    ...
    List<BaseEntity> entityList = map.get("list");
    sfe.className(classPrefix + "ListStruct");
    if (CollectionUtils.isEmpty(entityList)) {
      entityList = map.get(classPrefix + "ResponseData");
  ...
  public static void parseBaseEntity(Map<String, List<BaseEntity>> map, BaseEntity baseEntity,
    ...
    if (OBJECT_TYPE_SET.contains(type)) {
      String className;
      if ("list".equals(snakeProperty)) {
        className = GeneratorUtils.toUpperCaseFirstOne(classPrefix + "ListStruct");
      } else {
        className = GeneratorUtils.toUpperCaseFirstOne(baseEntity.getCamelProperty() + "Struct");
      }
